...
public class EndedWar {
	...
	HashMap<Guild, Integer> lootCollected = new HashMap<Guild, Integer>();
	HashMap<Guild, Integer> moneyCollected = new HashMap<Guild, Integer>();
	static ArrayList<EndedWar> wars = new ArrayList<EndedWar>();
	...
	public void removeItem(ItemStack i, Guild g) {
		if(loot.get(g) != null) {
			loot.get(g).remove(loot.get(g).indexOf(i));
	...
	public static ArrayList<EndedWar> getByGuild(Guild g) {
		ArrayList<EndedWar> guildWars = new ArrayList<EndedWar>();
		for(EndedWar war : wars) {
