...
public class Trade {
	...
	Trader requesterTrader;
	Trader requestedTrader;
	...
	public static ArrayList<Trade> activeTrades = new ArrayList<Trade>();
	...
	public Trade(GuildPlayer oneTrader, GuildPlayer requestedTrader) {
		...
		expire = new BukkitRunnable() {
			@Override
			public void run() {
				activeTrades.remove(activeTrades.indexOf(Trade.this));
				...
				if(requestedTrader.getPlayer().getPlayer().isOnline()) {
					Player onRequested = (Player) requestedTrader.getPlayer().getPlayer();
