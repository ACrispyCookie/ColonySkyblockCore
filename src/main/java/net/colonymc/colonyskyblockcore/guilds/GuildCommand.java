...
public class GuildCommand implements CommandExecutor {
	...
	HashMap<Player, Long> confirmDelete = new HashMap<>();
	final HashMap<Player, Long> confirmLeave = new HashMap<>();
	static final ArrayList<Guild> creatingIsland = new ArrayList<>();
	static final ArrayList<Player> bypasses = new ArrayList<>();
	static final ArrayList<GuildInvite> invitations = new ArrayList<>();
	static final ArrayList<GuildAllyRequest> allyRequests = new ArrayList<>();
	static final ArrayList<Player> toggledOnGuildChat = new ArrayList<>();
	...
	private void handleGuildTop(Player p, String[] args) {
		...
		for(int i = 0; i < Guild.getSortedTopGuild(15).size(); i++) {
			...
			if(i == 0) {
				topGuilds = new StringBuilder("\n  &5&l» &f#" + (i + 1) + " &d" + g.getName() + " &f- &d" + g.getLevel() + " Power Level");
			}
			else {
				topGuilds.append("\n  &5&l» &f#").append(i + 1).append(" &d").append(g.getName()).append(" &f- &d").append(g.getLevel()).append(" Power Level");
			}
		}
		...
		if(args.length == 1) {
			...
			for(int i = 1; i < args.length; i++) {
				if(i + 1 == args.length) {
					...
					messageToSend.append(args[i]).append(" ");
				}
			}
			if(!StringUtils.isEmptyOrWhitespaceOnly(messageToSend.toString())) {
	...
	private void handleGuildList(Player p, String[] args) {
		...
		StringBuilder playerList = new StringBuilder(ChatColor.translateAlternateColorCodes('&', "\n      &fMembers: &d"));
		for(int i = 0; i < allPlayers.size(); i++) {
			...
			if(i == g.getMemberUuids().keySet().size() - 1) {
				...
				if(toAdd.isOnline()) {
					playerList.append(g.getGuildPlayer(toAdd).getRole().color).append(toAdd.getName()).append(" ").append(ChatColor.GREEN).append("■");
				}
				else {
					playerList.append(g.getGuildPlayer(toAdd).getRole().color).append(toAdd.getName()).append(" ").append(ChatColor.RED).append("■");
				}
			}
			else {
				OfflinePlayer toAdd = allPlayers.get(i).getPlayer();
				if(toAdd.isOnline()) {
					playerList.append(g.getGuildPlayer(toAdd).getRole().color).append(toAdd.getName()).append(" ").append(ChatColor.GREEN).append("■").append(g.getGuildPlayer(toAdd).getRole().color).append(", ");
				}
				else {
					playerList.append(g.getGuildPlayer(toAdd).getRole().color).append(toAdd.getName()).append(" ").append(ChatColor.RED).append("■").append(g.getGuildPlayer(toAdd).getRole().color).append(", ");
				}
			}
		}
		StringBuilder allies = new StringBuilder(ChatColor.translateAlternateColorCodes('&', "\n      &fAllies: &7(None)"));
		StringBuilder enemies = new StringBuilder(ChatColor.translateAlternateColorCodes('&', "\n      &fEnemies: &7(None)"));
		ArrayList<Guild> relations = new ArrayList<>(g.getRelations().keySet());
		for(int i = 0; i < relations.size(); i++) {
			if(i + 1 == relations.size()) {
				if(g.getRelation(relations.get(i)) == Relation.ALLY) {
					if(allies.toString().equals(ChatColor.translateAlternateColorCodes('&', "\n      &fAllies: &7(None)"))) {
						allies = new StringBuilder(ChatColor.translateAlternateColorCodes('&', "\n      &fAllies: &d"));
					}
					allies.append(relations.get(i).getName());
				}
				else if(g.getRelation(relations.get(i)) == Relation.ENEMY) {
					if(enemies.toString().equals(ChatColor.translateAlternateColorCodes('&', "\n      &fEnemies: &7(None)"))) {
						enemies = new StringBuilder(ChatColor.translateAlternateColorCodes('&', "\n      &fEnemies: &d"));
					}
					enemies.append(relations.get(i).getName());
				}
			}
			else {
				if(g.getRelation(relations.get(i)) == Relation.ALLY) {
					...
					allies.append(relations.get(i).getName()).append(", ");
				}
				else if(g.getRelation(relations.get(i)) == Relation.ENEMY) {
					if(enemies.toString().equals(ChatColor.translateAlternateColorCodes('&', "\n      &fEnemies: &7(None)"))) {
						enemies = new StringBuilder(ChatColor.translateAlternateColorCodes('&', "\n      &fEnemies: &d"));
					}
					enemies.append(relations.get(i).getName()).append(", ");
				}
			}
		}
		header.send();
			...
			if(g != null) {
				if(!g.equals(Guild.getByPlayer(p))) {
					...
					for(int i = 0; i < allPlayers.size(); i++) {
						...
						if(i == g.getMemberUuids().keySet().size() - 1) {
							OfflinePlayer toAdd = allPlayers.get(i).getPlayer();
							playerList.append(g.getGuildPlayer(toAdd).getRole().color).append(toAdd.getName());
						}
						else {
							OfflinePlayer toAdd = allPlayers.get(i).getPlayer();
							playerList.append(g.getGuildPlayer(toAdd).getRole().color).append(toAdd.getName()).append(", ");
						}
					}
					StringBuilder allies = new StringBuilder(ChatColor.translateAlternateColorCodes('&', "\n      &fAllies: &7(None)"));
					StringBuilder enemies = new StringBuilder(ChatColor.translateAlternateColorCodes('&', "\n      &fEnemies: &7(None)"));
