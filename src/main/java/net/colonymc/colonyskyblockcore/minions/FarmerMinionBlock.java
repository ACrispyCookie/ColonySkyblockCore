	...
	protected boolean isInRightArea() {
		for(int x = 2; x > -3; x--) {
			if(loc.clone().add(x, 0, 2).getBlock().getType() != m.getBlocksNeeded() && loc.clone().add(x, 0, 2).getBlock().getType() != Material.AIR && 
					(m.getBlocksNeeded() != Material.SOIL || loc.clone().add(x, 0, 2).getBlock().getType() != Material.DIRT) &&
					(m.getBlocksNeeded() != Material.SOIL || loc.clone().add(x, 0, 2).getBlock().getType() != Material.GRASS)) {
				return false;
			}
		}
		for(int z = 2; z > -3; z--) {
			if(loc.clone().add(-2, 0, z).getBlock().getType() != m.getBlocksNeeded() && loc.clone().add(-2, 0, z).getBlock().getType() != Material.AIR && 
					(m.getBlocksNeeded() != Material.SOIL || loc.clone().add(-2, 0, z).getBlock().getType() != Material.DIRT) &&
					(m.getBlocksNeeded() != Material.SOIL || loc.clone().add(-2, 0, z).getBlock().getType() != Material.GRASS)) {
				return false;
			}
		}
		for(int x = -2; x > 3; x++) {
